...
public class RuntimeEventBus {
    ...
    public RuntimeEventBus(String lingId) {
        this.lingId = lingId;
    }
    ...
    public <E extends RuntimeEvent> Subscription subscribe(Class<E> eventType, Consumer<E> handler) {
        ...
        log.debug("[{}] Subscribed to {}", lingId, eventType.getSimpleName());
    ...
    public void publish(RuntimeEvent event) {
        log.debug("[{}] Publishing event: {}", lingId, event.getClass().getSimpleName());
            ...
            if (listener.eventType.isInstance(event)) {
                try {
                    ...
                    log.error("[{}] Error handling event {}: {}",
                            lingId, event.getClass().getSimpleName(), e.getMessage(), e);
                }
            }
        }
    }
    ...
    public void clear() {
        listeners.clear();
        log.debug("[{}] All subscriptions cleared", lingId);
    }
