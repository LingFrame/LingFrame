...
public class ServiceRegistry {
    ...
    public ServiceRegistry(String lingId) {
        this.lingId = lingId;
    }

    /**
     * 注册事件监听（由 LingRuntime 调用）
     */
    public void registerEventHandlers(RuntimeEventBus eventBus) {
        ...
        log.debug("[{}] ServiceRegistry event handlers registered", lingId);
    }

    private void onInstanceUpgrading(RuntimeEvent.InstanceUpgrading event) {
        log.debug("[{}] Instance upgrading, clearing proxy cache", lingId);
        clearProxyCache();
    }

    private void onRuntimeShuttingDown(RuntimeEvent.RuntimeShuttingDown event) {
        log.debug("[{}] Runtime shutting down, clearing all caches", lingId);
        clear();
    }
    ...
    public boolean registerService(String fqsid, Object bean, Method method) {
        ...
        try {
            ...
            if (old != null) {
                log.warn("[{}] Service {} was overwritten", lingId, fqsid);
                return false;
            }

            log.debug("[{}] Registered service: {}", lingId, fqsid);
            return true;
    ...
    public boolean unregisterService(String fqsid) {
        ...
        if (removed != null) {
            log.debug("[{}] Unregistered service: {}", lingId, fqsid);
            return true;
        }
        return false;
    }
    ...
    public InvokableService getServiceRequired(String fqsid) {
        ...
        if (service == null) {
            log.error("[{}] Service not found: {}", lingId, fqsid);
    ...
    public void clear() {
        ...
        log.debug("[{}] Cleared registry: {} services, {} proxies",
                lingId, serviceCount, proxyCount);
    }
    ...
    public void clearProxyCache() {
        int proxyCount = proxyCache.size();
        proxyCache.clear();
        log.debug("[{}] Cleared proxy cache: {} proxies", lingId, proxyCount);
    }
