...
// import com.lingframe.api.exception.LingInvocationException;Kernel;
...
// @Disabled("Disabled due to V0.3.0 core refactoring breaking changes. To be
// updated by user.")
...
// when(containerFactory.create(eq("Ling-a"), any(),
...
// any())).thenReturn(container);
//
// assertThrows(RuntimeException.class, () -> lingManager.installDev(definition,
// lingDir));
// }
//
// @Test
...
// lingManager.installDev(definition, invalidDir));
// }
//
// @Test
...
// LingContainer container = createMockContainer();
// when(containerFactory.create(eq(lingId), any(),
// any())).thenReturn(container);
//
// lingManager.installDev(definition, lingDir);
// }
...
//
// @Test
// @DisplayName("reload 不存在的灵元应该静默处理")
...
// List<LingInstance> instances =
// runtime.getInstancePool().getActiveInstances();
...
// doThrow(new RuntimeException("Container start
// failed!")).when(containerB).start(any());
// when(containerFactory.create(eq("Ling-b"), any(),
// any())).thenReturn(containerB);
//
// // 安装崩溃灵元
// assertThrows(RuntimeException.class, () -> lingManager.installDev(defB,
// lingDirB));
//
// // 验证灵元 A 不受影响
...
//
// @Test
// @DisplayName("灵元容器停止异常不应影响其他灵元卸载")
...
// doThrow(new RuntimeException("Container stop
// failed!")).when(containerA).stop();
// when(containerFactory.create(eq("Ling-a"), any(),
// any())).thenReturn(containerA);
// lingManager.installDev(defA, lingDirA);
//
...
//
// @Test
// @DisplayName("ClassLoader 创建失败不应影响其他灵元")
...
//
// @Test
// @DisplayName("安全验证失败不应影响其他灵元")
// void securityVerificationFailureShouldNotAffectOtherLings() throws
// IOException {
...
// managerWithVerifier.installDev(defB, lingDirB));
//
// // 验证 A 不受影响
...
// failed!")).when(container2).start(any());
// when(containerFactory.create(eq("Ling-a"), any(), any()))
// .thenReturn(container2);
//
// LingDefinition def2 = createDefinition("Ling-a", "2.0.0");
// assertThrows(RuntimeException.class, () -> lingManager.installDev(def2,
// lingDir));
//
// // 验证旧版本仍然可用
// LingRuntime runtime = lingManager.getRuntime("Ling-a");
...
// when(container.isActive()).thenReturn(true);
// doNothing().when(container).start(any());
// doThrow(new RuntimeException("Stop failed for " +
// lingId)).when(container).stop();
// when(containerFactory.create(eq(lingId), any(),
// any())).thenReturn(container);
//
...
// when(containerFactory.create(eq("good-ling"), any(),
// any())).thenReturn(containerGood);
...
// // reload 时容器创建失败
// LingContainer container2 = mock(LingContainer.class);
// doThrow(new RuntimeException("Reload
// failed!")).when(container2).start(any());
// when(containerFactory.create(eq("Ling-a"), any(), any()))
// .thenReturn(container2);
//
...
// doThrow(new RuntimeException("Event publish
// failed")).when(eventBus).publish(any());
//
// File lingDir = createLingDir("Ling-a");
// LingDefinition def = createDefinition("Ling-a", "1.0.0");
// LingContainer container = createMockContainer();
...
// e.getCause().getMessage().contains("Event"));
// }
// }
// }
//
...
//
// @Test
// @DisplayName("包含 System.exit 的灵元应该被拒绝")
...
// LingContainer container = createMockContainer();
// when(containerFactory.create(eq("evil-ling"), any(),
// any())).thenReturn(container);
//
// // 应该被安全检查拒绝
...
//
// assertThrows(RuntimeException.class, () -> secureManager.installDev(evilDef,
// evilDir));
