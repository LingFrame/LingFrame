package com.lingframe.example.test.service.impl;
...
import com.lingframe.example.test.service.TestService;
...
public class TestServiceImpl implements TestService {
    ...
    @Override
    public Optional<TestDTO> queryUser(String userId) {
        log.info("queryUser (cache miss), userId: {}", userId);
        String sql = "SELECT * FROM t_test WHERE id = ?";
        try {
            TestDTO user = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(TestDTO.class), userId);
    ...
    @Override
    public List<TestDTO> listUsers() {
        log.info("listUsers");
        return jdbcTemplate.query("SELECT * FROM t_test", new BeanPropertyRowMapper<>(TestDTO.class));
    }
    ...
    @Override
    public TestDTO createUser(String name, String email) {
        log.info("createUser, name: {}, email: {}", name, email);
        String sql = "INSERT INTO t_test (name, email) VALUES (?, ?)";
        ...
        return new TestDTO(generatedId, name, email);
    }
    ...
    @Override
    public TestDTO updateUser(String id, String name, String email) {
        log.info("updateUser, id: {}, name: {}, email: {}", id, name, email);
        String sql = "UPDATE t_test SET name = ?, email = ? WHERE id = ?";
        int updated = jdbcTemplate.update(sql, name, email, id);
        if (updated == 0) {
            log.warn("User not found for update: {}", id);
            return null;
        }
        return new TestDTO(id, name, email);
    }
