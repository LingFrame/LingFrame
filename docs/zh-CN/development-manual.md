...
此约束修复不统一两类权重：全局契约仍消费 provider 权重，限定灵元入口仍消费实例路由策略。现有零权重、标签与回退规则不等价于可靠禁止接流，独立接流门禁尚未提供。升级时需检查以前依赖显式版本静默回退的调用，按明确失败处理。
...
Dashboard 一键回滚及启动恢复均一次替换整表，移除表外旧覆盖；恢复解析失败不留下部分权重，历史存储的越界值仍按原规则截断。查询各行共用一份覆盖快照，但注册候选与运行期资格不属于同一原子时刻。原子性仅覆盖单 JVM、单契约的运行期权重表，不涵盖阶段推进、持久化、注册变更或跨作用域事务；存储仍为尽力写入，失败记录日志，不能据此保证重启后的比例。限定灵元的版本策略、接流屏障、命令幂等及实际调用修订审计仍需独立交付。
