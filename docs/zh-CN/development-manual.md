...
**显式目标路由契约**：`lingId:service` 与裸契约加 `targetLingId` 均限定在该灵元范围内；显式 `targetVersion` 必须在选择前收窄候选，不可用时返回路由失败，不能静默调用其他版本。裸契约的 provider 选择同样遵守版本约束。预解析实例必须与入口声明一致，自定义 `TrafficRouter` 只能返回候选集合中的实例。公共 starter 在用户已提供 `TrafficRouter` 时退让，两个实例选路入口共用注入的实现。

两类权重保持不同作用域：全局契约消费 provider 权重；限定灵元入口未配置局部策略时沿用实例路由，显式配置后消费局部版本策略。升级时需检查以前依赖显式版本静默回退的调用，按明确失败处理。零权重与禁止接流分开表达。

**原子 provider 权重覆盖**：灵核控制代码通过 `ProviderWeightRouter.getWeightSnapshot(contractId)` 读取不可变 `ProviderWeightSnapshot`，再以 `replaceProviderWeights(contractId, snapshot.getRevision(), weights)` 一次替换该契约的完整覆盖表。键使用 `ProviderDescriptor.providerKey()`；本层不校验注册存在性，调用方必须从注册信息取得目标。新接口拒绝空标识、空权重及不在 `[0,100]` 的值，总和无需为 100。遗漏的键恢复注册默认值，空表清除全部覆盖，**不是未列出即禁止接流的完整策略**。普通加权选择一次只读取一份覆盖表；标签优先、单候选与全零回退保持现有语义。

旧 `setProviderWeight`、`clearProviderWeight` 和卸载驱逐经同一写入口发布，旧 setter 仍截断权重，多次调用仍是多次提交。修订绑定路由器代次及契约，只可相等比较；过期、跨契约和路由器重建前的修订抛 `ConcurrentModificationException`，不自动覆盖重试。清空后不保留无限增长的墓碑，因此空作用域修订也可能因其他契约发布而失效；重读并重新决策即可。写操作复制当前覆盖索引，读侧不持写锁，适合控制面低频发布；大规模高频更新需另行压测。

Dashboard 一键回滚及启动恢复均一次替换整表，移除表外旧覆盖；恢复解析失败不留下部分权重，历史存储的越界值仍按原规则截断。查询各行共用一份覆盖快照，但注册候选与运行期资格不属于同一原子时刻。原子性仅覆盖单 JVM、单契约的运行期权重表，不涵盖阶段推进、持久化、注册变更或跨作用域事务；存储仍为尽力写入，失败记录日志，不能据此保证重启后的比例。命令幂等、策略持久化闭环及实际调用修订审计仍需独立交付。

**限定灵元的原子版本策略**：使用 `new LingRoutingScope(lingId, contractId)` 精确定位注册契约，通过 `ProviderWeightRouter.getLingVersionPolicy(scope)` 读取不可变快照，再调用 `replaceLingVersionPolicy(scope, snapshot.getRevision(), versionWeights)` 替换完整版本表。修订与路由器代次、作用域绑定，过期更新拒绝，不自动重试。权重取值 `[0,100]`，无需合计为 100；未列出的版本不参与选路，已配置的空表拒绝全部版本，只有 `clearLingVersionPolicy(scope, revision)` 才恢复既有实例路由。版本是否已注册、就绪及方法资格在调用时校验，发布本身不保证目标可用。

`lingId:contractId` 与裸契约加 `targetLingId` 使用同一局部策略，不扩大到其他灵元，也不与全局覆盖权重相乘。普通请求按正权重分流，单候选或全部零权重均不绕过此规则；明确版本、预解析实例及匹配标签可以定向到表内零权重版本，但不能进入表外版本。入口名称必须精确匹配契约，不猜测别名。提供方法签名时还检查实例方法目录，避免同版本重载代次继承其他实例的方法资格。先选版本，再在该版本的合格实例中选代次，副本数量不放大版本权重。一次调用固定一份策略，跟踪信息包含修订、所选版本和原因；这不是持久化审计或最终业务结果证明。

**实例接流屏障**：`LingInstance.setAcceptNewRequests(false)` 禁止该具体实例接收新的受管调用，`isAdmissionDisabled()` 查询配置；默认允许。该字段由实例准入锁保护，与生命周期状态、默认实例、能力授权分离，不修改 FSM。路由候选、预解析和自定义路由结果均受检查，标签、单候选、默认回退和零权重均不能绕过。全部候选禁用时明确路由失败；全局 provider 选择仍可能选到不可用版本并失败，不保证自动重选其他 provider。重新允许接流不会使非 READY 实例可用。

禁用返回与最终在途注册形成顺序：此前已登记调用允许完成，此后登记被拒绝。内置 IPC 调用器、受治理的灵核 AOP 和 Web 执行入口执行最终准入检查。AOP 凭据覆盖方法执行区间；Servlet 异步请求保持在途直到完成通知，超时或错误通知本身不表示执行结束。初始过滤器返回与异步完成都发生后才释放。监听登记异常会记录错误并保留在途信息，避免误报排空；此时需结合实际容器状态处置，不能将长期在途误认成清理完成。模拟不登记真实在途。自定义 `LingServiceInvoker` 必须遵守 `beginInvocation` / `completeInvocation` 配对契约；绕过治理的调用、主动豁免路径及方法返回后的独立任务不在此保证内。

上述策略与准入均为单 JVM 内存能力。整灵元驱逐清除其局部策略；当前 Dashboard 存储不保存局部策略，重启需上层重新发布。实例禁用针对具体对象，重载新实例不会继承旧实例禁用配置。`LingRuntime.getInstanceSnapshots()` 提供实例 ID、版本、生命周期状态、默认实例标记、接流许可和在途数量；卸载结果提供操作标识、清理完成与类加载器 GC 证据的分离字段。调用上下文提供结构化 `RoutingDecision`，包含灵元、版本、实例 ID、策略修订和选路原因，但不会跨重启持久化。跨作用域事务、命令去重、持久化操作历史和耐久审计仍由上层负责。

**卸载排空与重试契约**：默认生命周期引擎的全量卸载检查活跃与濒死实例的同一快照，等待模式下只有通过排空的实例才能进入回收；显式启用 `forceDrainOnTimeout` 时保留超时强制策略。按版本卸载涵盖同版本的全部代次；该引擎的具体实例卸载只处理指定对象，仍有同版本实例时保留其 provider 索引。等待策略超时后保留待回收实例，重试仍须检查在途调用。排空采用单调时钟；中断保留中断位并中止操作，不解释成强制卸载授权。零等待预算也先核对是否已经空闲。

**事务准备的资源作用域**：事务活跃检查、数据源枚举、连接提取/检查及下游执行共用清理作用域。任一步失败，只撤销本层成功压入的连接帧，不清空父事务连接或回滚信号，不关闭、提交或回滚上游连接；异常继续交给根事务所有者处理。
...
- `GovernanceConfigRestorer` 在启动时先恢复迁移阶段及候选元数据，再按契约恢复已存储的权重覆盖；旧灰度 `percent` 格式向后兼容映射。
- **诚实边界**：`ProviderWeightRouter` 本身只维护内存，Dashboard 服务可通过 `GovernanceStorage` 尽力持久化覆盖表。直接调用路由器不会自动落库；写入或恢复失败时必须重新核对运行期事实。阶段与权重没有跨存储事务，不能声称「重启前后状态完全一致」。
