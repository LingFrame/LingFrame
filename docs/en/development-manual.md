...
Weight scopes remain separate: global contracts use provider weights; ling-scoped entries use existing instance routing unless a local version policy is configured. Zero weight and admission denial are separate concepts. During upgrade, migrate callers that previously relied on silent fallback from an explicit version to handle routing failure.

**Atomic provider weight overrides**: LingCore control code reads an immutable `ProviderWeightSnapshot` through `ProviderWeightRouter.getWeightSnapshot(contractId)`, then calls `replaceProviderWeights(contractId, snapshot.getRevision(), weights)` to replace the complete override table for that contract. Obtain keys from `ProviderDescriptor.providerKey()`; this layer does not validate registration membership. The new API rejects blank identifiers, null weights and values outside `[0,100]`; weights need not sum to 100. Omitted keys revert to registered defaults, and an empty table clears overrides. This is **not a complete admission policy that excludes unlisted providers**. A weighted selection reads one override table; label priority, single-candidate selection and all-zero fallback retain their existing behavior.

Legacy `setProviderWeight`, `clearProviderWeight` and unload eviction publish through the same writer. The legacy setter still clamps values; repeated calls are separate commits. Revisions bind to a router generation and contract and support equality comparison only. Stale, foreign-contract and previous-router revisions throw `ConcurrentModificationException`, without automatic overwrite retries. Empty scopes retain no growing tombstone history, so unrelated publications may also invalidate an empty scope's revision; reread and reconsider the update. Writes copy the current override index; reads take no writer lock. This targets low-frequency control operations; large, frequent updates require separate load testing.

Dashboard rollback and startup restoration each replace one complete table, removing obsolete overrides. Parse failures leave no partial restoration; out-of-range historical values retain legacy clamping. Query rows share one override snapshot, but registry membership and live eligibility are not one atomic observation. Atomicity covers only the runtime weight table for one JVM and one contract, excluding phase changes, persistence, registration changes and cross-scope transactions. Persistence remains best effort with logged failures and does not guarantee restored traffic proportions. Command idempotency, reliable policy persistence and actual-invocation revision auditing remain separate work.

**Atomic ling-scoped version policies**: identify an exact registered contract with `new LingRoutingScope(lingId, contractId)`. Read `ProviderWeightRouter.getLingVersionPolicy(scope)`, then replace the complete version table through `replaceLingVersionPolicy(scope, snapshot.getRevision(), versionWeights)`. Revisions belong to a router generation and scope; stale writes fail without automatic retry. Weights range from 0 to 100 and need not sum to 100. Omitted versions are excluded. A configured empty table denies all versions; only `clearLingVersionPolicy(scope, revision)` restores legacy instance routing. Registration, readiness and method eligibility are checked at invocation time; publication does not guarantee availability.

Both `lingId:contractId` and a bare contract with `targetLingId` consume this policy without crossing into another Ling or multiplying global overrides. Ordinary requests use positive weights, including single-candidate cases; all-zero tables fail. Explicit versions, pinned instances and matching labels may select a listed zero-weight version, but never an omitted version. Contract names must match exactly, without alias inference. When a method signature is available, instance method catalogs prevent one deployment generation from inheriting another generation's eligibility. Routing selects a version before a qualified instance, so replica counts do not multiply its weight. Each invocation pins one policy snapshot. Traces include revision, selected version and reason; these are not durable audits or proof of business completion.

**Instance admission barrier**: `LingInstance.setAcceptNewRequests(false)` denies new governed invocations on that exact instance; `isAdmissionDisabled()` reads the setting, which defaults to false. An instance admission lock owns this setting independently of lifecycle state, default membership and capability authorization. Candidate filtering, pinned targets and custom router results enforce it. Labels, single-candidate selection and fallback cannot bypass denial. All-disabled candidates fail explicitly. Global provider routing can still select an unavailable version and fail; automatic provider reselection is not guaranteed. Re-enabling admission cannot make a non-READY instance available.

Final in-flight registration is ordered against disabling: previously registered work may finish; later admission is rejected. Built-in IPC invokers, governed LingCore AOP and Web execution enforce final admission. AOP leases cover method execution. Servlet asynchronous requests remain in flight until completion; timeout and error notifications alone do not release them. Both the initial filter frame and asynchronous execution must finish before release. Listener registration failures log an error and retain in-flight state instead of falsely reporting an idle instance; operators must reconcile it with container state. Simulation registers no real invocation. Custom `LingServiceInvoker` implementations must pair `beginInvocation` with `completeInvocation`. Ungoverned calls, explicitly exempt paths and detached tasks after method return are outside this guarantee.

These are single-JVM, in-memory facilities. Whole-Ling eviction removes local policies. Current Dashboard persistence does not store them; upper layers must republish after restart. Admission settings belong to the exact instance and are not inherited by replacement instances. `LingRuntime.getInstanceSnapshots()` exposes instance ID, version, lifecycle status, default membership, admission and in-flight count. Uninstall results expose an operation ID and separate cleanup-complete and class-loader-GC evidence fields. The invocation context exposes a structured `RoutingDecision` with Ling, version, instance ID, policy revision and reason; it is not durable across restarts. Cross-scope transactions, command deduplication, durable operation history and durable auditing remain upper-layer responsibilities.
...
- `GovernanceConfigRestorer` restores migration phases and candidate metadata before restoring stored weight overrides for each contract.
- **Boundary**: `ProviderWeightRouter` is in-memory only. Dashboard services optionally persist override tables through `GovernanceStorage` on a best-effort basis; direct router calls do not persist automatically. Failed writes or restoration require reconciliation with runtime facts. Phase and weight persistence are not one transaction and do not guarantee complete consistency across restarts.
