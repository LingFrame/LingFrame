...
This correction does not merge weight scopes: global contracts still use provider weights, while ling-scoped entries use instance routing. Existing zero-weight, label and fallback rules are not a reliable admission ban. A separate admission gate is not yet available. During upgrade, migrate callers that previously relied on silent fallback from an explicit version to handle routing failure.
