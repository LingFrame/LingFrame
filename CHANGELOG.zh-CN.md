...
- **开发效率提升**：
    ...
    - 开发模式下支持单元安装后自动激活，无需手动操作。
    - 集成 SpringDoc (Swagger) 支持，实现 API 自动分组（核心、单元、宿主）。

### 🛠 重构与优化

- **全局术语重构**：将所有“插件 (Plugin)”相关表述统一更名为“单元 (Ling)”，将“宿主 (Host)”更名为“灵核 (LingCore)”，确保概念高度一致。
- **隔离性增强**：优化 `SmartServiceProxy` 和 `InvocationExecutor`，强化单元边界审计能力。
- **基础设施 SPI**：针对 `StorageService` 和 `CacheService` 代理进行了稳定性优化。

### 🐛 问题修复

- **内存泄漏缓解**：通过系统性清理 Spring 缓存及 Jakarta EL/Objenesis 静态引用，尽可能缓解并优化了单元热重载过程中的 ClassLoader 内存泄漏问题。
