...
版本：`lingframe-dependencies` → `revision=0.4.7`。
代号：**含章**（0.4 版本线小版本更新，沿用既有代号）。

完整发布说明：[0.4.7 发布说明](docs/release/0.4.7-release-notes.md)。

### 治理结果与生命周期稳定性

- 修复 `GOVERN_ONLY` 将准入探针误计为真实业务成功或失败的问题；真实业务结果由适配层在终态统一回灌，避免流量指标和熔断结果重复结算。
- 延长异步调用准入凭据到 `CompletionStage` / `Future` 任务终态，统一处理成功、异常和取消，并确保结果只回报一次。
- 修复熔断器降级与恢复状态的运行时闭环，真实失败进入降级后可在成功结果回灌时恢复。
- 使运行时协调器的启动、停止具备幂等性；启动中断时回滚已注册监听器，避免重复监听与资源残留。
- 修复 JAR 缓存处理的 JDK 8 兼容性，避免通过不存在的协议级 API 影响双栈构建。

### 发布物可靠性

- Dashboard 明确排除本地 Tailwind 构建工具、输入样式和更新脚本，避免它们进入运行包与 `sources.jar`，降低 Maven Central 发布物体积。

## [V0.4.6] - 2026-09-22

版本：`lingframe-dependencies` → `revision=0.4.6`。
代号：**含章**（0.4 版本线小版本更新，沿用既有代号）。

完整发布说明：[0.4.6 发布说明](docs/release/0.4.6-release-notes.md)。

### 路由与运行时治理

- 支持契约级权重原子覆盖、修订校验与控制面批量发布，补齐原子路由发布入口及结构化实际命中事实。
- 支持实例级接流管控、局部版本路由和实例代次运行状态快照；路由命中、卸载和治理操作结果均关联实例代次并持久化。
- 拒绝不可接流的全局 Provider，统一显式版本路由约束，并修复事务清理与卸载重试。
- 延长异步调用准入至任务终态，支持实例代次范围的卸载清理。
- 弹性治理组件默认开启，并支持在运行时配置中统一关闭或分别关闭超时、舱壁、限流和熔断；关闭后清理对应运行时状态，类加载器及生命周期治理不受影响。
- Spring Boot `LingFrameProperties` 与虚拟灵元配置同步暴露上述开关，确保应用配置能够完整映射到治理内核。

### Dashboard 与可观测性

- 展示真实路由命中、实例代次、卸载清理、持久化恢复、备份、审计和治理操作结果。
- 新增控制面审计事实的持久化与查询，并将前端操作接入原子路由和真实运行证据。
- 默认关闭高风险演练能力，完善双栈构建脚本的 Spring Boot 3 输出目录处理。
- Dashboard 中英文界面版本标识更新为 `V0.4.6`。
...
- **开发效率提升**：
    ...
    - 集成 SpringDoc (Swagger) 支持，实现 API 自动分组（核心、灵元、灵核）。
